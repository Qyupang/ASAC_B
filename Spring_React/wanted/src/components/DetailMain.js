...
const DetailMain = (props) => {
  ...
  const tagList = dummy.main[mainContentNum].tag.map((tags) => (
    <button className="Detail_Main_Text_TagButton_Style">#{tags}</button>
  ));
  const skillList = dummy.main[mainContentNum].skill.map((skills) => (
    <div className="Detail_Main_Text_Skill">{skills}</div>
  ));
